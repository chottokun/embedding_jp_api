...
async def benchmark_endpoint():
    ...
    async with httpx.AsyncClient(
        ...
        for batch_size in [1, 8, 32, 64]:
            ...
            resp = await client.post(
                "/v1/embeddings",
                json={"model": "cl-nagoya/ruri-v3-30m", "input": batch_input},
            )
            ...
            print(
                f"  • Batch Size {batch_size:2d}: {elapsed * 1000:6.1f} ms | Throughput: {batch_size / elapsed:6.1f} items/sec | Token Rate: {total_tokens / elapsed:7.1f} tok/sec"
            )
        ...
        print(
            f"  • Rerank (5 docs) Avg Latency: {statistics.mean(rerank_lats):.2f} ms (P50={statistics.median(rerank_lats):.2f} ms, P95={sorted(rerank_lats)[int(len(rerank_lats) * 0.95)]:.2f} ms)"
        )
