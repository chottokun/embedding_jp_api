...
### 3.6. パフォーマンスとセキュリティの最適化
 ...
 - **推論セマフォ制御 (`MAX_CONCURRENT_INFERENCES`)**: 推論処理の同時実行数を制限する `AsyncThreadSemaphore` を配備。キュー待機タイムアウト（`INFERENCE_SEMAPHORE_TIMEOUT_SECONDS=30`）を設け、過負荷時の GPU/CPU 枯渇・CUDA OOM を防ぎ、溢れたリクエストには適切に `503 Service Unavailable` を返却します。
 - **マルチAPIキー & 個別レート制限**: `API_KEYS` 環境変数により複数クライアントの個別キー管理をサポート。タイミング攻撃耐性認証（`secrets.compare_digest`）と、クライアント毎の個別レート制限（毎分リクエスト数）を提供します。
 ...
 - **スレッドプールによる並列実行**: AnyIO ワーカースレッドプールを使用して同期的な推論処理を分離し、FastAPI の非同期イベントループが死活監視や Prometheus メトリクス要求を常に健全に処理できるようにしています。
...
#### CPUモード（セマフォ制御稼働、10並行ワーカー負荷テスト）

| メトリクス | 実測値 | 判定・所見 |
| :--- | :---: | :--- |
| **スループット (req/s)** | **`3.42 req/s`** | CPU環境における最大効率 |
| **中央値レイテンシ (P50)** | **`874 ms`** | 通常滞留時のレスポンス |
| **95% レイテンシ (P95)** | **`6,321 ms`** | セマフォによる安全なキューイング |
| **成功率** | **`100.0% (50/50)`** | エラー率 0.0% (クラッシュ・OOM ゼロ) |

- **最大処理能力**: 合計 **2.5 〜 3.5 req/s** 程度が、CPUのみの構成でエラーなく安定処理できる限界値の目安です。
...
起動時間を短縮するため、および完全オフライン（エアギャップ）環境で利用するために、全モデル（テキスト＋マルチモーダル重み＋従来型リランカー＋Logit Gate因果言語モデル）を一括ダウンロードし、ロード検証（Dry-Run）を実施できます。
