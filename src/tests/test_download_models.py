...
def test_download_models_bge_visualized_failure(
    ...
    assert any(
        "bge-visualized-m3 のダウンロードに失敗しました" in msg for msg in printed_messages
    )
...
@patch("builtins.open", new_callable=mock_open)
@patch("app.download_models.MODELS_FILE")
def test_download_models_with_verify_offline_success(
    mock_models_file, mock_file, mock_yaml, mock_snapshot, mock_get_model, mock_print
):
    """Test download_models with verify_offline=True succeeds when get_model loads models."""
    mock_models_file.exists.return_value = True
    ...
    mock_snapshot.assert_called_once_with(repo_id="model1")
    mock_get_model.assert_called_once_with("model1", device="cpu")
    printed_messages = [
        str(call.args[0]) for call in mock_print.call_args_list if call.args
    ]
    assert any("全モデルのオフライン完全ロード検証に成功しました！" in msg for msg in printed_messages)


@patch("builtins.print")
@patch("sys.exit")
@patch("app.models.get_model")
def test_verify_offline_loading_failure(mock_get_model, mock_exit, mock_print):
    """Test verify_offline_loading exits with code 1 if model loading fails."""
    ...
    with pytest.raises(SystemExit) as excinfo:
        verify_offline_loading(["model_fail"])
